...
func TestTransferTx(t *testing.T) {
	...
	fmt.Printf(">>> before transfer: %+v %+v\n", account1.Balance, account2.Balance)
	...
	for i := 0; i < n; i++ {
		...
		_, err = store.GetEntryByAccountID(context.Background(), fromEntry.AccountID)
		require.NoError(t, err)
		...
		_, err = store.GetEntryByAccountID(context.Background(), toEntry.AccountID)
		require.NoError(t, err)
